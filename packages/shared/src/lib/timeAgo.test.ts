...
describe('getTimeAgo', () => {
  ...
  it('타임존이 없는 값은 UTC로 해석한다(서버가 Z 없이 UTC를 주는 경우)', () => {
    freezeNow('2026-07-23T12:00:00Z')
    // 'Z'가 붙은 값과 안 붙은 같은 값이 같은 결과여야 한다.
    expect(getTimeAgo('2026-07-23T11:30:00')).toBe('30분 전')
  })

  it('미래 시각(음수 차이)은 방금 전으로 방어한다', () => {
    freezeNow('2026-07-23T12:00:00Z')
    expect(getTimeAgo('2026-07-23T12:05:00Z')).toBe('방금 전')
  })
})
