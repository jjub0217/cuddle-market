...
const eslintConfig = defineConfig([// 1. Next.js 기본 규칙들 적용
// Core Web Vitals 최적화 규칙 (React, Hooks, 접근성, import 규칙 포함)
...nextVitals, // TypeScript 권장 규칙
...nextTs, // 2. 무시할 파일/폴더 설정
globalIgnores([
  '.next/**', // Next.js 빌드 결과물
  'out/**', // 정적 빌드 결과물
  'build/**', // 빌드 폴더
  ...
  '.claude/**',
  // 브라우저로 재려고 잠깐 만드는 스크립트 자리. 다 쓰면 지우지만 **잊어도 게이트가
  // 안 깨지게** 여기서 뺀다 — 저장소 뿌리의 .cjs 는 켜진 규칙 41개로 검사받아서,
