...
export default function MyScreen() {
  ...
  //
  // 예전에는 (home)과 (my)가 둘 다 그룹이라 주소에 안 나타났고, 둘 다 index를 갖고 있어서
  // 두 화면의 주소가 똑같이 '/'였다. 그래서 마이에서 '/'로 가라고 하면 지금 있는 자리를
  // 가리키게 되어 아무 일도 일어나지 않았다 — 실기기에서 두 번 헛돈 자리다.
  //
  // 지금은 마이가 괄호를 떼어 /my 라(#1096) '/'로도 홈에 갈 수 있다. 그래도 그룹을 콕
  // 집어 둔다: '/'가 홈이라는 것은 **홈만 괄호 폴더로 남겨 둔 덕**이라, 괄호 배치가
  // 바뀌면 다시 어긋난다. 콕 집어 두면 그 배치와 상관없이 홈으로 간다.
