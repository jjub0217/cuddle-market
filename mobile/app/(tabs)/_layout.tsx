...
//    앱을 켰을 때 어느 탭이 열리는지는 **폴더 이름의 괄호 배치**가 정한다 —
//    아래 「홈 말고 네 탭은 괄호를 안 쓴다」를 봐라. `anchor` 는 거들 뿐이다(#1096).

/**
 * 앱을 열었을 때 어느 탭을 **기본 탭으로 삼는가** (#1096).
 *
 * ⚠️ **이것만으로는 첫 화면을 못 정한다.** 그 이야기는 이 블록 맨 아래에 있다.
 *    먼저 왜 이런 것이 필요했는지부터 적는다.
 *
 * ⚠️ **예전에는 채팅 탭이 열렸다.** 괄호로 감싼 폴더는 URL 에 안 들어가서
 *    다섯 탭이 다 `/` 에 해당했고, 그래서 알파벳순으로 걸렸다 —
