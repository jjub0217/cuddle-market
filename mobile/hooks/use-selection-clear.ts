 ...
 *
 * ⚠️ **올리지 마라.** 위험한 쪽은 「꾹 누르기가 느린 기기」가 아니라 **빠른 기기**다 —
 * 이 값이 꾹 누르기 기준보다 크면 선택이 시작되자마자 우리가 풀어 버린다.
 * 안드로이드의 기준은 500ms(`ViewConfiguration.getLongPressTimeout()`)이고 접근성 설정으로
 * **늘릴 수만** 있다. 그래서 300 은 아래로 200ms 여유를 둔 값이다.
 */
const TAP_MS = 300;
