...
export function tabGroupOf(source: string | string[] | undefined): TabGroup {
  ...
  if (Array.isArray(source)) {
    // useSegments()는 그룹을 괄호째 준다 — ['(tabs)', '(home)', 'products', '[id]']
    // ⚠️ **홈만 괄호 폴더다**(#1096). my 는 괄호를 뗐으므로 두 모양을 다 알아본다.
    const found = GROUPS.find(
      (group) => source.includes(`(${group})`) || source.includes(group)
    );
    return found ?? 'home';
  }
...
/**
 * `/(tabs)/my/products/12` 처럼 탭까지 적힌 주소
 *
 * ⚠️ **홈만 괄호 폴더다**(#1096). 다섯을 다 괄호로 감쌌더니 다섯이 전부 `/` 가 되어
 *    앱을 열면 채팅 탭이 열렸다. 그래서 홈만 남기고 넷은 괄호를 뗐다.
 */
export function productDetailHref(group: TabGroup, id: number | string): string {
  const segment = group === 'home' ? '(home)' : group;
  return `/(tabs)/${segment}/products/${id}`;
}
