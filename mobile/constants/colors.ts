...
export const colors = {
  ...
  // ── 선
  // 테두리는 둘이다. **테두리 혼자 무엇을 말하느냐**로 갈랐다.
  //
  // 입력칸은 이름표와 안내글이 이미 「여기 쓰는 칸이다」를 말해 준다.
  // 테두리는 칸의 경계만 알려주면 되므로 옅어도 된다.
  // 단추는 다르다 — 테두리가 사라지면 **누를 수 있다는 것 자체가 안 보인다.**
  //
  // 19바퀴(#786) 실기기에서 「취소 단추 테두리가 없다고 생각될 정도」라는 말이
  // 나와 둘을 갈랐다. 전에는 둘 다 #D1D5DB(1.47:1) 하나였다.
  outline: '#D2D3D5', // 입력칸·카드·뱃지. 당근마켓 입력칸에서 뽑은 값이다 (1.50:1)
  outlineStrong: '#8D8D8D', // 누를 수 있는 것. 3.32:1 — 기준(3:1)을 넘는 가장 옅은 회색
