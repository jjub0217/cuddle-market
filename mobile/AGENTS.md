...
| **고른 글자가 빈 곳을 눌러도 안 풀린다** | 안드로이드의 글자 선택은 **다른 뷰가 누름을 가져갈 때** 끝난다. 그런데 보통 `View` 는 누름을 안 잡아서 빈 곳을 눌러도 선택이 남는다 | 배경에 `<Pressable style={StyleSheet.absoluteFill} onPressIn={Keyboard.dismiss} accessible={false} />` 를 **깐다**. **누름을 잡는 것 자체가 일**이라 핸들러가 비어도 된다 — 글자를 다시 그릴 필요 없다. ⚠️ `onPress` 는 **손을 뗀 뒤**라 늦게 느껴진다. `onPressIn` 을 쓴다. ⚠️ 키보드도 같이 내려라 — 배경이 누름을 먼저 가져가면 `ScrollView` 의 `keyboardShouldPersistTaps="handled"` 가 못 내린다 |
| **`key` 를 바꿔 글자를 다시 그리기 (하지 마라)** | 2026-08-20 에 선택을 풀려고 `selectable` 글자에 `key={열쇠}` 를 걸었다. **필요 없었다** — 배경이 누름을 잡는 것만으로 풀린다. 다음 날 `key` 를 빼고 재서 확인했다 | 걸지 마라. 값도 비싸다 — 조각째 다시 그리면 **본문 사진의 가로세로 비가 되돌아가 튀고**, react-query 를 쓰는 목록은 **다시 받아 온다** |
| **selectable 글자의 조상이 누름을 잡으면** | **꾹 누르기가 통째로 죽는다.** 두 번 밟았다 — ① `<Pressable>` 로 감쌌을 때 ② 부모 `View` 에 `onStartShouldSetResponder` 를 달았을 때. 게이트는 둘 다 초록이었다 | 배경은 **형제로** 깔고 절대 감싸지 마라. ⚠️ 그 대신 **배경은 「그 위에 아무 뷰도 없는 자리」에서만 누름을 받는다** — 다른 글자가 차지한 자리(「7개월 전 · 동구」 같은 줄)에서는 안 풀린다. 그것까지 살리려면 **선택 대상이 아닌 글자만 골라** 감싸야 하는데, 화면마다 손이 가서 지금은 안 한다 |
| **expo-image 의 `source` 를 그 자리에서 만들기** | `source={cond ? { uri } : PLACEHOLDER}` 는 **다시 그릴 때마다 새 객체**다. expo-image 가 「사진이 바뀌었다」로 보고 큰 그림을 다시 그린다 — 화면이 다시 그려질 때마다 그 값을 치른다 | `useMemo` 로 붙잡는다. 2026-08-20 에 「글자 선택이 늦게 풀린다」의 범인이었다(그때는 다시 그리기를 쓰고 있었다) |
...
**줄 번호로 가리키지 않는다.** 이 저장소는 앱이 웹 문구·값을 베끼는 규칙이라 주석에서 웹을 자주 가리키는데, **웹이 몇 줄만 움직여도 조용히 어긋난다.** 2026-08-22(#1015)에 68군데를 다 이름으로 바꿨다 — 그 전에 `ProductsSection.tsx` 를 가리키던 여덟 개를 재 보니 **전부 틀려 있었다.**

```diff
- // 문구는 웹 ProductListHeader 그대로다 — ProductsSection.tsx:20.
+ // 문구는 웹 그대로다 — `ProductsSection.tsx` 의 `ProductListHeader`.
```

| 가리킬 것 | 어떻게 |
|---|---|
| **이름이 있다**(조각·함수·상수·타입) | `` `파일명` 의 `이름` `` — 이름은 파일이 움직여도 안 밀린다 |
| **이름이 없다**(문구·클래스값·조건) | **그 값을 주석에 직접 적는다.** 「문구는 웹과 같다 — "판매중"」처럼. 찾아가지 않아도 알 수 있어야 한다 |
| **덩어리를 가리킨다** | 무슨 일을 하는 곳인지 한 마디로. 「빈 목록 자리도 `SearchX` 아이콘을 연한 동그라미로 감싼다」 |

⚠️ **틀려도 아무것도 안 알려준다.** 타입체크·린트·시험 어디에도 안 걸린다. 그런데 앱이 웹을 베끼는 규칙이라, 어긋난 줄을 따라가면 **엉뚱한 코드를 보고 「웹은 이렇게 하는구나」로 잘못 배운다.**

⚠️ **앱→앱도 마찬가지다.** 같은 이유로 어긋난다.

⚠️ 옮겨 적을 때는 **적힌 줄을 믿지 말고 대상을 열어 확인한다.** #1015 에서 그렇게 하다가 설명 자체가 틀린 것도 바로잡았다 — 「웹도 `isLogin()` 일 때만 그린다」가 실제로는 `hasHydrated && isLogin()` 이었다.
