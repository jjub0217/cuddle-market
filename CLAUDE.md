...
```
docs/session-logs/2026-08-25-handoff.md      ← 앞으로 여기에 쓴다
                                                .gitignore 에 있어 **아예 안 올라간다**
docs/superpowers/plans/*-handoff.md          ← 2026-08-24 까지의 옛 것. 그대로 둔다
```

⚠️ **새 이어받기 문서를 `superpowers/plans/` 에 만들지 마라.** 거기 있는 12개는 예전에
올린 것이고, 지우지 않고 남겨 둔 것뿐이다(이력이 쓸모 있을 때가 있다 — 2026-08-25 에
「어제 문서가 낡았다」를 그 이력으로 찾았다).
...
**검토받을 사람이 없다.** 작업자가 한 사람이고 PR 을 열어 스스로 머지한다. 그런데도
`kimi` 리뷰가 일곱 번 돌았고 전부 「특이사항 없음」이었다 — 순수한 토큰 낭비다.

⚠️ **그리고 다른 기기에서 볼 일이 없다.**

```
맥미니   여기서 작업한다
맥북     이어받기 문서를 안 연다(2026-08-25 에 확인)
폰       **리모트 컨트롤로 맥미니를 조종한다** — 맥미니의 로컬 파일을 보는 것이다
```

즉 원격에 둘 까닭이 없다. **`session-logs/` 는 원래 이 문서들이 있던 자리**이고
(`.gitignore:63`), 어느 시점에 `superpowers/plans/` 로 옮겨 갔던 것을 되돌리는 것이다.

⚠️ **잃는 것도 알고 정했다.** 맥미니가 고장나면 이어받기 문서는 사라진다.
   코드는 원격에 있어 안전하지만 이 문서들은 그렇지 않다.

### 그 밖의 문서는

```
CLAUDE.md · mobile/AGENTS.md · .claude/skills/** · docs/superpowers/specs/**
  → 저장소에 올린다. **문서만 바뀌면 PR 없이 develop 에 바로 커밋·푸시한다**

  git add CLAUDE.md && git commit -m "docs: …" && git push origin develop

  ⚠️ `main` 은 안 맞춘다. 문서 때문에 `develop → main` 을 돌리지 마라
  ⚠️ 코드가 한 줄이라도 섞이면 **PR 로 간다** — 게이트가 머지 전에 막아 주는 것이
     PR 의 유일한 값어치이고, 그건 코드일 때만 뜻이 있다
  ⚠️ 커밋 메시지 규칙은 그대로다. `docs:` 만 이슈 번호가 면제고 `chore:` 는 아니다
```
...
## ⚠️⚠️ 앱 빌드를 돌리기 전에 **푸시부터** 한다

**EAS 는 깃이 아니라 로컬 작업본을 올려서 빌드한다.** 그래서 **안 밀어도 빌드가 되고,
폰에서는 고쳐진 것처럼 보인다.** 저장소에만 고침이 없다.

```
2026-08-27   괄호를 뗀 커밋을 만들고 **푸시를 안 한 채** EAS 빌드를 돌렸다
             → versionCode 5 에는 고침이 들어갔다   (폰에서 홈이 열린 것은 진짜였다)
             → 원격 브랜치는 앞 커밋에서 멈춰 있었다
2026-08-30   그 PR 을 머지했다. **「안 고쳐졌다」로 판정났던 앞 커밋만** 들어갔다
```

⚠️ **모든 표시가 초록이었다.** 폰에서 홈이 열리고, PR 은 Merged, 이슈는 Closed,
`git status` 도 깨끗했다 — **커밋만 하고 안 민 상태도 깨끗하다.**

**어떻게 알아챘나** — `main` 에 머지한 뒤 바뀐 파일이 3개뿐인 것이 이상했다.
36개 파일을 고쳤는데 그게 안 보였다. `git push origin main` **하기 전에** 멈췄다.

```bash
git branch -a --contains <커밋>     # 원격 브랜치가 안 나오면 안 민 것이다
```
...
```
① EAS 빌드 전에 반드시 push
② 머지한 뒤 **바뀐 파일 목록을 눈으로 본다.** 예상한 것이 거기 있는가
③ `git status` 가 깨끗한 것은 「다 올렸다」는 뜻이 **아니다**
```

⚠️ **빌드가 끝나면 `Commit` 값을 본다.** `eas-cli build:list` 가 찍어 주는데, 이 값이
원격의 `main`·`develop` 과 같아야 **저장소에 있는 코드가 빌드된 것**이다. EAS 는 깃이
아니라 로컬 작업본을 올리므로 이것이 유일한 증거다.

⚠️ **`eas submit` 은 쓰지 않는다.** 첫 production 이라 **구글 서비스 계정 키**를 물어본다.
AAB 를 내려받아 플레이 콘솔에 직접 올린다.

⚠️ **「지원 기기 406개가 빠진다」 경고는 늘 뜬다.** 네이버 지도 SDK 때문이고 진짜 전화는
71개(-1%)뿐이다. **「무시하고 계속하기」가 맞다.**

### 출시 노트 — 불릿은 **`·`** (가운뎃점)

```
· 판매요청 글의 가격에 「희망」을 붙여 판매 상품과 구분했습니다.
```

⚠️ **`•`(U+2022 불릿)가 아니라 `·`(U+00B7 가운뎃점)다.** 둘은 다른 글자이고, 섞어 쓰면
스토어에서 **버전마다 모양이 다르게** 보인다. 2026-09-01 에 `•` 로 써 드렸다가 지적받았다.

```
쓰는 것    이번 versionCode 에 **실제로 담긴 앱 변경**만. 커밋으로 확인한다
             git log --oneline <지난 빌드 커밋>..<이번 커밋> -- mobile/ packages/shared/
안 쓰는 것  서버만 고친 것 — 앱을 안 올려도 이미 적용돼 있어서 「올려야 고쳐진다」로 읽힌다
             이슈 번호·파일 이름 — 테스터가 읽는 글이다
             디자인 조정 상세 — 보이면 되는 것이지 설명할 일이 아니다
길이       언어당 500자. 한국어(ko-KR) 하나만 채우면 된다
```

⚠️ 테스터에게 **부탁할 것**이 있으면 출시 노트가 아니라 **메시지로** 보낸다.
   노트는 「무엇이 바뀌었나」만 적는 자리다.
...
⚠️ **이것은 PR 을 머지했을 때의 이야기다.** 문서만 바뀌어 `develop` 에 바로 밀었을 때는
   **`main` 을 안 맞춘다**(위 「이어받기 문서는 저장소에 안 올린다」 절 끝). 문서 때문에
   `develop → main` 을 돌리지 마라 — `main` 은 코드 릴리스용이다.
