---
name: create-issue
description: GitHub 이슈 생성 및 브랜치 생성. "/create-issue", "이슈 생성", "이슈 만들어" 요청에 사용.
---
...
⚠️ **템플릿 머리말(frontmatter)을 이슈 본문에 넣지 마라.** `.github/ISSUE_TEMPLATE/*.md` 를 열면
맨 위에 `---` 로 감싼 부분(`name:` · `about:` · `title:` · `labels:` · `assignees:`)이 있다.
그건 **GitHub 이 템플릿을 고르는 데 쓰는 것**이지 이슈 본문이 아니다.

```
본문에 넣을 것    ## 🐞 버그 설명  아래의 머리글들만
넣지 말 것        --- 로 감싼 맨 위 다섯 줄
```

⚠️ 2026-08-24 에 이슈 초안을 쓰면서 **두 번** 이것이 섞여 들어갔다(올리기 전에 잡았다).
템플릿을 통째로 베껴 오면 머리말까지 딸려 온다 — **머리글부터 복사해라.**
