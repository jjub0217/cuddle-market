...
- [x] 사진을 바꾸면 반영된다
- [x] **사진을 넣은 뒤 다른 것만 고쳐 저장해도 사진이 안 지워진다** (전체 교체 함정)
- [x] **소셜 가입 화면에서 저장해도 사진·소개글이 안 지워진다** (Task 1-5 가 지키는 것)
- [x] 소셜 계정이면 비밀번호 묶음이 **안 보인다**
- [x] 이메일 계정이면 비밀번호를 바꿀 수 있다
- [x] 현재 비밀번호가 틀리면 알려준다
- [x] 새 비밀번호와 확인이 다르면 알려준다
