...
> **완료** (2026-08-06). 커밋 8개 · 시험 345 → 375.
> 실기기에서 5건이 나왔다 — 아래 「실기기에서 나온 것」 참고.
...
- [x] **1-1.** 시험 먼저. 조건이 쿼리에 실리는지, **없으면 아예 안 실리는지**
...
- [x] **1-2.** `fetchProducts(page)` → `fetchProducts(params)` 로 바꾼다.
      ⚠️ 부르는 자리(홈)도 같이 고친다. 안 그러면 홈이 깨진다.
- [x] **1-3.** ⚠️ **「전체」는 빈 값이다.** `'ALL'` 같은 글자를 보내면 서버가 그런 종류를 찾는다.
      값이 `null`·`''` 이면 쿼리에서 **빼야** 한다.
- [x] **1-4.** `npx jest lib/products` · 홈이 전과 같이 도는지 눈으로
- [x] **1-5.** 커밋
...
- [x] **2-1.** 조각으로 옮길 것 — `useInfiniteQuery` · `HomeRow` · 3상태(로딩·오류·빈) ·
      `FlatList` · `onEndReached` · `ListFooter`
- [x] **2-2.** 화면에 남길 것 — `SafeAreaView` · `AppHeader` · **떠 있는 「상품 등록」 단추**
      ⚠️ 단추는 로그인했을 때만 보이고, 그만큼 목록 아래를 비운다. 그 값을 `bottomInset` 으로 넘긴다.
      게스트에겐 안 비운다 — 늘 비우면 목록 끝이 허전하게 뚫린다(2026-08-04에 잡은 것).
- [x] **2-3.** 시험 — 목록이 나온다 · 로딩 · 오류 + 다시 시도 · 빈 화면
- [x] **2-4.** `pnpm gate:mobile`
- [x] **2-5.** **실기기에서 홈이 전과 똑같은지** 확인 (무한스크롤 · 찜 · 단추 자리)
- [x] **2-6.** 커밋
...
- [x] **3-1.** 선택지는 **`@cuddle/shared` 에서 가져온다** — `PET_TYPE_OPTIONS`(9) ·
      `CATEGORY_OPTIONS`(8). 새로 적지 마라.
- [x] **3-2.** 줄 둘. 각각 가로 스크롤. 맨 앞에 **「전체」**(값은 `null`).
- [x] **3-3.** 하나만 고른다. **같은 것을 다시 누르면 풀린다**(웹과 같다).
- [x] **3-4.** ⚠️ 색·모양을 새로 짓지 마라. `components/places/category-tabs.tsx` 가
      같은 알약이다 — 그 값을 그대로 쓴다.
- [x] **3-5.** ⚠️ **잘린 게 보이게** 오른쪽 끝에 여백을 준다. 딱 맞으면 뒤에 더 있는 걸 모른다.
- [x] **3-6.** 시험 — 고르면 알린다 · 다시 누르면 `null` 로 알린다 · 「전체」가 맨 앞이다
- [x] **3-7.** 커밋
...
- [x] **4-1.** 조각 안에 `petType`·`category` 상태를 둔다. 맨 위에 `ProductFilterRow`.
- [x] **4-2.** ⚠️ **조건이 바뀌면 목록을 처음부터 받는다.** `queryKey` 에 조건을 넣는다.
...
- [x] **4-3.** 시험 — 필터를 바꾸면 그 조건으로 다시 부른다
- [x] **4-4.** `pnpm gate:mobile` · 실기기에서 필터가 도는지
- [x] **4-5.** 커밋
...
- [x] **5-1.** 헤더 오른쪽에 돋보기. `AppHeader` 의 `right` 자리를 쓴다.
      ⚠️ 알림 벨이 이미 거기 있는지 확인하고, 있으면 나란히 둔다.
- [x] **5-2.** 검색 화면 — `ScreenHeader`(뒤로) + 입력칸 하나. 들어오면 **바로 초점**.
      문구는 웹과 같다: **「원하는 반려동물 용품을 검색해보세요」**
- [x] **5-3.** 넣으면 결과 화면으로. ⚠️ **빈 검색어면 넘어가지 않는다** — 홈과 같아진다.
- [x] **5-4.** ⚠️ `_layout.tsx` 에 등록하는 걸 빠뜨리지 마라 — 네이티브 헤더가 라우트 이름을
...
- [x] **6-1.** `useLocalSearchParams` 로 검색어를 받아 `<ProductListView keyword={...} />`
      **그게 전부다.** 필터는 조각 안에 있어 저절로 따라온다.
- [x] **6-2.** 헤더에 **검색어를 제목으로** 보여준다. 누르면 검색 화면으로 되돌아간다(고치기).
- [x] **6-3.** 커밋
...
- [x] 홈이 **전과 똑같다** (무한스크롤 · 찜 · 「상품 등록」 단추 자리)
- [x] 알약 두 줄이 목록을 너무 아래로 밀지 않는다 — **목록이 몇 개나 보이는지**
- [x] 알약을 고르면 목록이 바뀌고, 다시 누르면 풀린다
- [x] 알약이 **잘려 보인다** (뒤에 더 있는 걸 알 수 있다)
- [x] 돋보기 → 검색 → 결과. 결과에서도 알약이 돈다
- [x] **뒤로 오면 홈의 필터가 그대로** 남아 있다
- [x] 필터를 바꾼 뒤 스크롤해도 목록이 안 뒤섞인다
...
## 실기기에서 나온 것 (게이트가 초록인 상태에서)

```
검색 줄이 세로로 쌓임        계획서가 공용 ScreenHeader 를 쓰라고 해서. 한 줄로 고침
검색 결과에 홈 문구가 나옴    조각을 빼면서 홈의 빈 화면 문구가 따라감
빈 화면 문구를 새로 지음      웹(ProductsSection.tsx:136)에 이미 있었는데 안 찾음
필터 걸고 나갈 길이 없음      로고·홈 탭 재탭으로 처음 상태 되돌리기 추가
마지막 카드가 기기 바에 가림   루트 화면인데 edges 에 'bottom' 을 안 줌
```

## 시험이 비어 있던 곳 (마커로 드러남)

```
조건이 바뀌면 처음부터   계획서가 경고했는데 시험이 없었다. 필터를 눌러보는 시험을 추가
빈 화면·오류 아이콘      370개 중 아무도 안 봤다. 이모지로 되돌려도 전부 통과했다
```

**게이트가 초록이라고 지켜지는 게 아니다.** 무엇을 안 덮는지 마커로 재봐야 안다.
