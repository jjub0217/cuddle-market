...
> ## ✅ 끝났다 (2026-08-09)
>
> **아래 체크박스는 안 채웠다.** 실제로 한 일이 계획과 여러 곳에서 달라져,
> 칸을 채우면 오히려 거짓말이 된다. **무엇을 했는지는 커밋 일곱을 본다.**
>
> ```
> 399e71a0  과제 1  Button 높이를 h-* 로
> 49fb8b6e  과제 2  입력칸·단추를 40 으로
> 5ca06c96          h-* 로 커진 칩들을 배포본 크기로 되돌림
> b061fc98  과제 3·4  입력칸을 공용 기본값 하나로
> 4fbb7640  과제 5   단추·탭·뱃지를 공통값으로  (범위가 늘었다 — 아래 참고)
> 1171bc0e          홈 카테고리 아이콘 잘림 (이 브랜치가 만든 회귀)
> a93dc7ef          홈 스켈레톤을 실제 크기로 (#686 부터 있던 문제)
> 644c273f  과제 6   앱 주 단추 48
> ```
>
> ### 계획과 달라진 것
>
> | 계획 | 실제 |
> |---|---|
> | 모바일 48 / 데스크탑 40 (반응형) | **고정 40.** 실기기에서 48이 입력칸보다 커 어색했다 |
> | 과제 4의 `border-blue-100` 이 입력칸 | **AlertBox 였다.** 안 건드렸다 |
> | 과제 7 「넓게 퍼진 곳 훑기」 26곳 | **거의 없어졌다.** 높이를 고정하며 원래 값 그대로가 됐다 |
> | 과제 5는 `buttonClass` 한 곳 | **범위가 늘었다.** 이슈의 진짜 목적이 「세 매체 공통 조각 맞추기」로
>   분명해지면서 모달 단추·탭 글자·뱃지·단추 글자까지 함께 봤다 |
>
> ### 계획이 놓쳤던 곳 (실측으로 찾음)
>
> ```
> 생년월일 칸        공용 Input 을 안 써서 38px·옛 색으로 남을 뻔했다
> 상품 설명 textarea 베이지 테두리가 클래스로 박혀 있어 grep 에 안 걸렸다
> 로그인 화면 칸     배경이 혼자 크림(bg-primary-50)이고 글자가 12 였다
> 로그인 모달 「로그인」  Button 이 아니라 Link 라 높이가 44 로 어긋나 있었다
> HomeHero 검색 단추  생 button 이라 글자를 줄이자 높이가 40→36 으로 따라 내려갔다
> ```
>
> **여기서 배운 것:** 공용 조각을 고쳐도 **그 조각을 안 쓰는 곳은 안 따라온다.**
> 손으로 그린 자리를 실측으로 찾아야 한다. 게이트는 높이·색을 못 잡는다.
