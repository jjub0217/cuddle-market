...
갤럭시 실기기 + 웹 모바일 폭을 나란히 놓고 확인했다. 위 완료 기준 중 **세 줄은 배포 후에만**
확인할 수 있어 남겼다.

```
확인 완료   헤더·벨·점 · 비로그인 벨 없음 · 알림 목록 · 모두 읽음 · 스켈레톤
            찜 알림 → 상품 상세 · 웹 오버레이 네 자리 · 웹 두 페이지 헤더 통일
            앱 tsc·lint·jest(134) / 웹 tsc·eslint·build

배포 후      무한스크롤            실제 구글 계정으로 로그인해야 알림이 충분히 쌓인다
            채팅·커뮤니티 알림     위와 같은 이유
            웹 알림 목록 탭바 가림  배포된 화면에서만 재현된다
```

### 계획서가 틀렸던 것 넷

실기기에서 드러났다. 다음 바퀴 설계 때 되풀이하지 않으려고 남긴다.

| 무엇 | 무엇이 틀렸나 | 어떻게 알았나 |
|---|---|---|
| 로고 상자 | `120x32`에 `contentFit:'contain'`을 걸라고 했는데, 그림 비율이 2.31이라 상자(3.75) 안에서 **좌우 23px씩 빈칸**이 깔렸다. 그게 「로고 왼쪽만 더 뜬다」로 보였다 | 스크린샷 픽셀 실측 + PNG 알파 채널 측정 (좌우 투명 여백은 0이었다) |
| 아이콘 세트 | MaterialIcons → Feather로 한 번 고쳤지만 Feather도 웹과 다른 그림이었다. 종 치마 끝이 뾰족하고(Lucide는 반지름 1로 굴린다) 햄버거가 더 좁다(y=6/12/18 대 5/12/19) | 아이콘 파일의 path 좌표를 열어 대조 · 스크린샷 실측 14.0dp 대 16.0dp |
| 마이 탭 줄 아이콘 | 웹 모바일에 아이콘이 있는데 앱에 없었다. **웹을 데스크탑 폭으로만 보고** 옮긴 탓이다 — 데스크탑 메뉴(`myPageIconMap`)는 `hidden md:flex`라 모바일에서 안 보이고, 모바일에는 따로 아이콘 달린 목록이 있다 | 사용자 지적 |
| 햄버거 | 스펙에 **한 줄도 없었다.** 급히 만들면서 웹 `MobileNavigation`의 위쪽 버튼 두 개(로그아웃/마이페이지 · 로그인/회원가입)를 못 봤다 | 사용자 지적 |

### 되풀이한 실수 하나

**웹의 그 화면을 실제로 열어보지 않고 앱을 만든 뒤, 지적받으면 그럴듯한 이유를 붙였다.**
이번 바퀴에만 세 번이다 — 빈 화면 문구 · 햄버거 버튼 · 마이 탭 줄 아이콘.
셋 다 코드에는 「웹과 같은 결」이라는 주석이 달려 있었으나 사실이 아니었다.

앱에 화면을 새로 만들 때는 **웹의 같은 화면을 모바일 폭으로 먼저 열어보고** 옮긴다.
문구·아이콘·색은 지어내지 말고 반대쪽에서 가져온다.
...
| 앱 색 토큰 (#786) | 햄버거 버튼·마이 줄의 색을 웹 브랜드 토큰에 맞추는 일. 한 곳만 옮겨 두면 그때 다시 걷어내야 해서 통째로 미뤘다. 웹의 `--color-primary-200`은 흰 글자 대비가 1.59:1이라 그때 함께 손봐야 한다 |
