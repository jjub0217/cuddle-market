...
> 웹도 같이 고치는 게 맞지만 이번 범위 밖이다 — 신고·차단과 무관한 결함이라 따로 이슈로 뗀다.
...
사용자 신고가 **실제로 되는 것은 확인했다**(프로필에 「신고완료」가 뜬다). 그건 GraphQL 경로라 `@RequestBody`와 무관하다.

**상품 신고가 실제로 되는지는 아직 모른다.** `@RequestBody`에 FormData를 보내면 보통 415가 난다. 이번 범위 밖이라 별도로 본다 — **앱은 둘 다 REST를 직접 부르므로 이 갈림과 상관없다.**
