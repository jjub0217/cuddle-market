...
### 미리보기 화면에는 되돌릴 수 없는 동작을 붙이지 않는다

웹 데스크탑 마이페이지의 **대시보드**는 판매 내역 · 구매 내역 · 찜한 상품을 탭으로 두고, 각각 카드 몇 장만 보여준 뒤 「전체보기」로 넘긴다. 이 미리보기는 세 탭 모두 홈과 같은 `ProductCard`(찜 버튼 있음)를 쓴다. 반면 「전체보기」로 들어간 판매 · 구매 목록은 `MyList`(찜 버튼 없음, ⋮ 상태변경 · 수정 · 삭제)를 쓴다.

**이 차이를 유지한다.** 미리보기는 "훑어보는 곳"이고 전용 목록은 "관리하는 곳"이다. 되돌릴 수 없는 동작(상태 변경 · 삭제)을 미리보기에 노출하면 잘못 눌렀을 때 손해가 크다. 찜은 다시 누르면 되돌아가므로 미리보기에 있어도 괜찮다.

> **5바퀴에 적용**: 상태 변경 · 삭제는 전용 목록 화면에만 붙인다. 앱에 대시보드를 만들게 되면 거기엔 붙이지 않는다.

**부수효과 하나**: 대시보드의 판매 내역 카드에도 찜 버튼이 있어, 자기 물건을 찜하면 찜 목록과 개수 통계에 자기 물건이 섞인다. 버그가 아니라 이 제품이 이미 택한 설계다 — 상세 화면에서도 자기 물건에 찜이 된다(§5). 막으려면 서버 · 웹 · 앱 세 곳을 다 손봐야 해서 지금은 두고 본다.

### 「내 활동」 — 이름을 통일했다

웹 모바일 폭의 카드 제목이 `나의 활동`이었는데 상수(`MY_PAGE_NAV`의 `nav-activity`)는 `내 활동`이었다. 상수를 정본으로 삼아 **`내 활동`으로 통일**했다. 같은 화면의 「내 상품 관리」와도 결이 맞는다. 같은 종류로 어긋나 있던 `차단 유저` → `차단한 사용자`도 함께 맞췄다.

앱에 「내 활동」 카드를 넣을 때 이 이름을 쓴다. 앱에 아직 없는 것은 **대시보드 · 내 활동(내 글 · 댓글) · 차단한 사용자** 셋이다.
