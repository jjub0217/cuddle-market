...
> ⚠️ **말 바꿈(2026-08-25)**: 원래 이 문서는 「응답을 뭉갠다」라는 말을 썼다. 뜻이 바로
> 읽히지 않아 **「같은 답만 주게 한다」**로 고쳤다. 뜻은 그대로다 — 회원이든 아니든
> 화면이 똑같이 답해서 밖에서 구분할 수 없게 만드는 것이다.
...
이슈 본문은 ①화면이 같은 말만 하게 하기 → ③계정 찾기 순으로 적혀 있는데, **③ 이 먼저**다
(2026-08-25 사람 결정).

```
지금        비밀번호 찾기가 「계정 찾기」 노릇을 겸한다 — 화면이 새기 때문에
            → 화면만 같은 말을 하게 하면 구멍은 남고 친절만 잃는다

계정 찾기를 먼저 만들면
            「가입 방법을 잊은 사람」이 메일로 답을 받는 정식 경로가 생긴다
            → 그러면 화면이 같은 말만 해도 잃을 친절이 없다 = 딜레마가 사라진다
```

그래서 **1단계 = 계정 찾기**(이번에 만들었다), **2단계 = 같은 말만 하게 하기·응답 시간
맞추기**(설계만 적어 둔다, §8). 1단계도 **서버 엔드포인트가 붙어야 실제로 굴러간다** — §5-5.

⚠️ 「바꿀 신호」(소셜 문의·처방침 점검)를 기다릴 일이 아니라고 본 근거: 그 신호는
**「화면이 같은 말만 하게 할 때가 됐나」**를 재는 것이지 **「친절을 더할 때가 됐나」**가 아니다.
...
| 로그인 (앱) | `session.ts:39-41` 이 400·401 을 `InvalidCredentialsError` 하나로 묶는다. 서버 문구를 안 싣는다 |
...
### A안 — 프론트만 같은 답을 주게 한다 (문구·단계·링크를 하나로)
...
| 비밀번호 찾기가 같은 답만 주게 하기 (§8) | 열거가 그대로 뚫려 있다 |
...
오는 자리라 갈라 말해도 안 샌다. 오히려 같은 말만 하게 하면 「왜 안 바뀌지」가 된다.
